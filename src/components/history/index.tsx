...
const getRecentSearches = async (setHistory: React.Dispatch<React.SetStateAction<never[]>>) => {
...
const renderHistory = (navigation: any) => {
    return ({ item }: { item: ILyricsCardProps }) => {
        ...
        return <LyricsCard artist={artist} song={song} lyrics={lyrics} onPress={goToScreen(navigation, 'Detail', item)} />
    }
}

const renderEmptyState = () => {
    return <View style={styles.imageContainer}>
        <Image style={styles.image} source={require('./../../assets/img/lyrics.png')} />
        <Text style={styles.emptyText}>{`There's nothing in your search history, start your first search by tapping the search option`}</Text>
    </View>
}
...
const History = (props: IHistoryProps) => {
    ...
    return <>
        {isArrayLength(history, 'greater', 0) ? <FlatList data={history} style={styles.container} renderItem={renderHistory(navigation)} keyExtractor={keyExtract} /> : renderEmptyState()}
    </>
}
